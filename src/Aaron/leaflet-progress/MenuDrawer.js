...
import { getBorderSelection, getTreatmentSelection, getSelectedDate } from "../../actions/index";
...
class MenuDrawer extends Component {
    ...
    handleTreatment(e, isInputChecked) {
        if (e.target.value == 'gratitude') {
            ...
            this.props.getTreatmentSelection({ gratitude: isInputChecked, intention: this.state.intention, pressure: this.state.pressure, other: this.state.other, opacity: this.state.opacity, retour: this.state.retour });
        } else if (e.target.value == 'intention') {
            this.setState({ intention: isInputChecked });
            this.props.getTreatmentSelection({ intention: isInputChecked, gratitude: this.state.gratitude, pressure: this.state.pressure, other: this.state.other, opacity: this.state.opacity, retour: this.state.retour });
        } else if (e.target.value == 'other') {
            this.setState({ other: isInputChecked });
            this.props.getTreatmentSelection({ other: isInputChecked, gratitude: this.state.gratitude, pressure: this.state.pressure, intention: this.state.intention, opacity: this.state.opacity, retour: this.state.retour });
        } else if (e.target.value == 'retour') {
            this.setState({ retour: isInputChecked });
            this.props.getTreatmentSelection({ retour: isInputChecked, intention: this.state.intention, gratitude: this.state.gratitude, pressure: this.state.pressure, other: this.state.other, opacity: this.state.opacity });
        } else {
            this.setState({ pressure: isInputChecked });
            this.props.getTreatmentSelection({ pressure: isInputChecked, gratitude: this.state.gratitude, intention: this.state.intention, other: this.state.other, opacity: this.state.opacity, retour: this.state.retour });
        }
    }
    ...
    handleChangeDate(event, index, value) {
        this.props.getSelectedDate(value);
        ...
        return (
            <div>
                ...
                >
                    ...
                    <div style={{ marginTop: '5vh', marginBottom: '2vh', marginLeft: '2vh' }}>
                        <h4 className='bulletPoint fiveMarginTop'>Choose a date</h4>
                        <DropDownMenu iconStyle={{ fill: 'red' }} value={this.state.chosenDate} onChange={this.handleChangeDate.bind(this)}>
                            <MenuItem value={'G_L_23'} primaryText="22,23-04-18" />
                    ...
                    <div style={{ marginTop: '5vh', marginBottom: '2vh', marginLeft: '2vh' }}>
                    <h5 className='bulletPoint fiveMarginTop'  >Distribution advancement </h5>

                    <div className="progress" style={{width: '90%',height: '20px'}}>
                        <div className="progress-bar progress-bar-success" role="progressbar" aria-valuenow=""
                            ariaValuemin="0" ariaValuemax="100" style={{width:(this.props.per*280/100)}}>
                            {this.props.per}  distributed 
                        </div> 

                    </div>
                    </div>
                    <div style={{ marginTop: '5vh', marginBottom: '2vh', marginLeft: '2vh' }}>
                    <h5 className='fiveMarginTop' ><b style={{color:'red'}}>{this.props.retourValue}</b> returned leaflet in <b style={{color:'red'}}>{this.props.vcNumberRetour}</b>  VC</h5>
                    <br/>
                    <h5  ><b style={{color:'red'}}>{this.props.totaldistriuted}</b> distributed leaflet(no return considered) in <b style={{color:'red'}}>{this.props.vcNumberreached}</b>  VC</h5>
...
function mapDispatchToProps(dispatch) {
    ...
    return bindActionCreators({ getBorderSelection, getTreatmentSelection, getSelectedDate }, dispatch);
}

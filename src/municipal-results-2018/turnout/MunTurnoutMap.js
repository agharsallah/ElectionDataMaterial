...
export default class MunTurnoutMap extends Component {
  ...
  componentWillMount() {
    ...
    axios({
      ...
      .then(response => {
        this.setState({ shape_mun: JSON.parse(response.data.data) });
      })
    ...
    axios({
      ...
      .then(response => {
        this.setState({ shape_gov: JSON.parse(response.data.data), shapeIsLoaded: true });
      })
      .catch(function (error) {
        console.log(error);
      });
  ...
  handleRadioFilter(filter, e) {
    ...
    if (filter == 'govLevel') {
      this.setState({  grades: [0, 34, 38], filter, checked });
  ...
  render() {
    ...
    return (
      <div className=''>
        <section className='row col-md-11 col-md-offset-1' >
        ...
        {this.state.shapeIsLoaded ? <Map maxZoom={9} center={[34.79, 10.18]} scrollWheelZoom={false} zoom={7} minZoom={5} style={{ height: "95vh", width: "100%", position: "relative", zIndex: this.props.mapZIndex, backgroundColor: "white" }}>
          <TileLayer
            url='https://api.mapbox.com/styles/v1/hunter-x/cixhpey8700q12pnwg584603g/tiles/256/{z}/{x}/{y}?access_token=pk.eyJ1IjoiaHVudGVyLXgiLCJhIjoiY2l2OXhqMHJrMDAxcDJ1cGd5YzM2bHlydSJ9.jJxP2PKCIUrgdIXjf-RzlA'
            attribution='&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> '
          />
          {this.state.filter == 'govLevel' ?
            <GeoJSON

...
export default class PartyMap extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      percentageSign: ' %', maxFilter: 100, minFilter: 0, munFilter: 'all', activeFilter: 'none', grades: []
    }
  }
  componentWillMount() {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.partyName != this.state.partyName) {
      ...
      this.setState({
        ...
        grades: nextProps.grades_votes, checked: [true, false], minFilter: 0, maxFilter: 100, munFilter: 'all', activeFilter: 'none',
        shapeIsLoaded: false
      });
    }
  ...
  handleRadioFilter(filter, e) {
    ...
    if (filter == 'perVotes') {
      this.setState({ grades: this.props.grades_votes, keyTitle: 'Results per votes percentage', percentageSign: ' %', minFilter: 0, maxFilter: 100, munFilter: 'all', activeFilter: 'none' });
    } else if (filter == 'perSeats') {
      this.setState({ grades: this.props.grades_seats, keyTitle: 'Results per seats number', percentageSign: '', minFilter: 0, maxFilter: 25, munFilter: 'all', activeFilter: 'none' });
    }
            ...
            <div className="col-lg-12 col-sm-2 col-sm-offset-2 col-lg-offset-1">
              <div className="well MenuShadow SideMenuePosition info-card-font">
                ...
                <section className='row col-md-12 '  >
                  ...
                  <div className='form-group' controlId="typeOfAssoc" onChange={this.handleMunType.bind(this)}  >
                    <select className='form-control' componentClass="select" placeholder="All" value={this.state.munFilter} >
